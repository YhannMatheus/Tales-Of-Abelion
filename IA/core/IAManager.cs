...
public class IAManager : MonoBehaviour
{
    ...
    [Header("CharacterManager Data (NPC)")]
    ...
    void Awake()
    {
        ...
        Debug.Log($"[IAManager] {characterData.characterName} inicializado como {CharacterManager.characterType}");
    }
    ...
    public void Die()
    {
        ...
        if (CharacterManager.characterType == CharacterType.Enemy)
        {
            HandleEnemyDeath();
        }
        else if (CharacterManager.characterType == CharacterType.Ally)
        {
            HandleAllyDeath();
        }
    ...
    {
        List<CharacterManager> nearbyPlayers = FindNearbyPlayers(xpDistributionRange);

        if (nearbyPlayers.Count > 0 && experienceReward > 0)
        {
            int xpPerPlayer = Mathf.RoundToInt(experienceReward / nearbyPlayers.Count);

            foreach (CharacterManager player in nearbyPlayers)
            {
    ...
    private List<CharacterManager> FindNearbyPlayers(float range)
    {
        List<CharacterManager> players = new List<CharacterManager>();
    ...
    private void OnDrawGizmos()
    {
        ...
        if (CharacterManager != null && CharacterManager.characterType == CharacterType.Enemy)
        {
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        if (characterData != null)
        {
            UnityEditor.Handles.Label(
                ...
                $"[{(CharacterManager != null ? CharacterManager.characterType.ToString() : "Unknown")}] {characterData.characterName}\n" +
